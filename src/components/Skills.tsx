  ...
  return (
    <section id="skills" className="relative min-h-screen flex items-center py-24 overflow-hidden">
      ...
      <div ref={ref} className="section-container relative z-10">
        ...
        >
          <p className="text-sm uppercase tracking-[0.3em] text-white/40 mb-3">Skills</p>
          <h2 className="text-3xl md:text-4xl font-semibold text-white">My Tech Stack</h2>
          ...
          {skills.map((skill, index) => (
            ...
            >
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-black/20">
                <img
                  src={skill.iconSrc ? skill.iconSrc : `https://cdn.simpleicons.org/${skill.slug}/${skill.color}`}
                  alt={`${skill.name} logo`}
                  className="h-6 w-6"
                  loading="lazy"
                  decoding="async"
